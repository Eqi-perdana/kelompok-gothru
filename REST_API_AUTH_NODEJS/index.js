...
const userRefreshToken = Datastore.create('UserRefreshTokens.db');
...
app.post('/api/auth/login/2fa', async (req, res) => {
  try {
     ...
     if (!userId) {
      return res.status(401).json({ message: 'The Provided temporary token is incorrect expired'})
     }
...
app.post('/api/auth/refresh-token', async (req, res) => {
  try {
    ...
    await userRefreshToken.remove({ _id: storedToken._id });
    await userRefreshToken.persistence.compactDatafile();
...
await users.persistence.compactDatafile(); 
      ...
      return res.status(200).json({ message: 'TOTP validated succeessfully' })
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
})
